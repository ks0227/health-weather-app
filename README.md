...
| ページ             | 内容                                                        |
| ------------------ | ----------------------------------------------------------- |
| トップページ       | Bootstrap carousel採用のナビゲーション　　　　　　　　　　  |
| 体調入力フォーム   | 気分・睡眠・症状をWebフォームから記録（前日以前のみ登録可） |
...
| フロントエンド | HTML / CSS / JavaScript / Bootstrap 5.3 / Bootstrap Icons / M PLUS 1p |
| イラスト       | unDraw（フリーSVGイラスト）                                           |
| ホスティング   | Render                                                                |
...
│   │   └── common.css       # 共通スタイル
│   └── images/
│       ├── undraw_clouds_bmtk.svg
│       ├── undraw_mobile-web_eqrb.svg
...
過去日付で登録した場合はOpen-Meteoの確定済みデータを使用しますが、当日の場合は1日分のデータが未確定のため分析の一貫性が保てません。そのため当日以降の登録を不可としています。
...
| ------------ | -------- | -------------------------------- |
| 有意差あり ✓ | p < 0.05 | 信頼性が高い                     |
| 傾向あり     | p < 0.10 | 傾向として参考になる             |
| 判定不能     | p ≥ 0.10 | データが増えると変わる可能性あり |
